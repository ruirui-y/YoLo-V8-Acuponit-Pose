...
def make_start_dir_provider(
    settings_store: SettingsStore,
    le_to_key: dict[QLineEdit, str],
) -> Callable[[QLineEdit], str]:
    """构造 Panel browse 起始目录 QSettings fallback 的纯 callable。

    返回值：callable(le) -> str
        - 命中 QSettings 上次保存的同 key 路径 -> 返回 str(path)
        - 未命中或 le 不在映射里 -> 返回 ""
          （Panel 自己补 _REPO_ROOT 兜底）
    ...
    def _provider(le: QLineEdit) -> str:
